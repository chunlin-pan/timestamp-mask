...
def timestamp(filename, time, duration, output_name, start_time=0, color='white', audio='on'):
    ...
    for i in second_range:
        text_clip = TextClip(set_time(time, i), fontsize=70,
                             color=color).set_duration(1)
    ...
    if type(start_time) in [tuple, list]:
        ...
        video = VideoFileClip(filename).subclip(
            start_time, start_time + duration)
    if audio == 'off':
        video = video.without_audio()
    final_text_clip = concatenate_videoclips(
        text_list).set_position((0.05, 0.85), relative=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-t', '--time', help='year, month, day, minute', type=str)
    ...
    parser.add_argument('-aud-off', '--audio-off',
                        help='turn off audio', action='store_true')
    parser.add_argument('-o', '--outputname', help='input xxx.mp4', type=str)
    args = parser.parse_args()

    filename = args.filename
    # [year, month, day, hour, minute, second]
    time = list(map(int, args.time.split(',')))[:6]
    start_time = args.starttime
    duration = args.duration
    audio = 'off' if args.audio_off else 'on'
    output_name = args.outputname
    timestamp(filename=filename, time=time, duration=duration,
              output_name=output_name, start_time=start_time, audio=audio, color='white')
